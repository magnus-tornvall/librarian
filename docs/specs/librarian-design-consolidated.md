...
12. **Memory correctness & maintenance** (added 2026-07-09 from an agent-memory literature round: LongMemEval knowledge-update lineage, Zep/Graphiti bi-temporal invalidation, SSGM "Memory Worth", SAGE/MemGuard write-time gating). Five sub-items, ordered by leverage; the first is the tripwire that tells us when the rest stop being deferrable:
