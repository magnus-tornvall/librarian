...
- **All surfaces route through the single registered bin (thin plugins, not self-contained).** The Claude Code plugin declares its four hooks + the stdio MCP server in `.claude-plugin/plugin.json` (no user settings-editing); the per-agent hook-payload mapping moves behind a `librarian hook <agent>` subcommand so a thin plugin needs only the bin, not a bundled `dist/`. OpenCode plugin resolves the CLI via config `bin`. Rejected: self-contained plugins that each bundle `dist/` (three code copies, three path regimes — the opposite of one validated bin).
  - **The Claude Code plugin carries no `version`, so installs track this repo's default branch (accepted pre-1.0).** Claude Code resolves a versionless git-sourced plugin to the commit SHA and updates from the branch, so a broken `main` is a broken plugin for everyone installed. Accepted deliberately while the only users are the author: pinning versions means a release cadence and a plugin↔bin skew to reason about, both of which buy nothing at one user. Trigger to add `version` + tags (`claude plugin tag` exists for this): the first external adopter — the same trigger as public cross-platform distribution below. This is the *same* product-repo coupling that #155's rejected generated-release-ref proposal was invented to avoid; recorded here so the two positions are visible rather than silently opposed. The difference that makes it acceptable here and not there: #155's coupling came with a build step, a tag namespace, and Bun cache-invalidation semantics to maintain, whereas this one is the absence of machinery.
  - **A declarative host manifest cannot express fallback, so the exit-0 hook-safety contract is enforced in the command string.** Each hook command is `librarian hook claude-code || true`. Claude Code treats a non-zero `UserPromptSubmit` hook as *blocking* (the prompt is erased), and a bare-PATH `librarian` may be any version the user has installed — including one predating the `hook` subcommand, which prints usage and exits 2. No change inside this repo can fix a binary already on a user's PATH, so the guard is the only place the invariant "instrumentation must never break the instrumented agent" can be made to hold by construction. In-process hosts (OpenCode) get this from code and need no guard.
