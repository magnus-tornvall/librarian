...
**Deferred, with named triggers (§15.1):** item 10's second inference provider +
SQLite-mirror exporter (T-03, after the first path proves out); item 11's first
non-agent source (T-01, `ContentEvent` + per-origin profile). Item 11 jumps *behind*
...
### 15.1 Trigger register

Every deferral in this design is gated on a **named condition**, and those conditions are
scattered across §5, §12, §14, and the prose below. The register indexes them so a trigger
can be *referenced* — from an issue, from a review, from the architecture model — instead of
re-described each time. A trigger id is durable: it outlives whatever work it eventually
unlocks, which is why it lives here and not on an issue.

The register is an index, not a second home: the reasoning stays where it already is, and
the "Reasoning" column says where. Nothing here records status — whether a trigger has
fired, and what was built when it did, belongs to GitHub issues.

| ID | Fires when | Unlocks | Reasoning |
|----|------------|---------|-----------|
| **T-01** | The first non-agent source is actually wanted | `ContentEvent` shape + per-origin distiller profiles | §5 (deleted/deferred), §12 item 11 |
| **T-02** | A genuinely different context arrives that a coding-session vocabulary would distil badly (an email source) | Mode system reconsidered; claude-mem's mode-file shape is the design to study | §5 |
| **T-03** | The first inference path proves out | Second inference provider + SQLite-mirror exporter (roadmap item 10) | §12 |
| **T-04** | The SQLite mirror lands | `Exporter` interface, shaped from the working `exportNoteToVault` signature | §15 |
| **T-05** | First external contributor, or first publish | Contracts barrel, `docs/extending.md`, conformance helpers; deferred `package.json` metadata | §14, §15 |
| **T-06** | Negative fixtures show distractors surviving hybrid + the relevance floor | Re-ranking pass (the escalation, not the first response) | §5, §15 |
| **T-07** | Endpoint friction proves real — a user who will not run Ollama wants hybrid | In-process ONNX embedding provider | §15 |
| **T-08** | `librarian stats` shows duplicate NOOPs against near-TTL notes at a meaningful rate | Corroboration-extends-TTL (12.5, parked); the lazy novelty-gate variant first | §5 |
| **T-09** | A real, flowing, note-granular outcome signal exists to calibrate against | Reopens outcome-linked note worth (12.3, closed — do not reopen without this) | §5 |
| **T-10** | A second machine | Per-machine log segments, `created_by_machine`, and the deterministic-ID revision-fork resolution rule | §15 |
| **T-11** | Two importers ingest the same curated file (likely fires *before* T-10) | Curated-vault sync collision rule | §15 |
| **T-12** | A second user | Team memory: separate shared store, per-peer recall weights, visibility structural not per-record | §15 |
| **T-13** | A second host that genuinely needs in-process code (a real Cursor/Zed/Aider plugin) | Packaging the OpenCode plugin file — two call sites, then generalise | §14 |
| **T-14** | First external adopter | Cross-platform distribution: per-OS/arch matrix, public download channel, signing/notarization, signed release feed | §15 |
| **T-15** | A secret or private record is discovered in the event log | `librarian purge-event-range --rewrite-segment` — rare, loud, audited outside memory | §15 |
| **T-16** | The first renderer change after real traces exist | Rendered block, or renderer version + hash, stored in the injection trace | §15 |
| **T-17** | Wanting spans in an external tracing backend, or something feels slow | OTLP export of diagnostics; timing/latency spans | §5, §15 |
| **T-18** | Size on disk matters | Log compaction/GC; gzip of closed segments | §15 |
| **T-19** | Demand for editing *in Obsidian* specifically | Vault-edit detected → prompt to convert into a human revision. Never silent region merging | §5 |

Each trigger appears in `docs/architecture/librarian.c4` as a `gate` element carrying its
id, so the frontier views and this register cannot drift apart without one of them being
visibly wrong.
