...
**Amendment since v4 (2026-07-19 — flag_note review, #106):**
- **§10.2/§12.2 amended — a validity-close applies only to revisions it post-dates (supersedes the earlier permanent-close semantics).** Superseded text: the index closed a note's `invalid_at` at the earliest supersession timestamp and *no later revision could re-open it*. That made the softest kill (a flag, 12.12) permanent while the strongest (a tombstone) was already revivable by a newer revision — backwards, and it stranded deterministic-ID notes (`project:{slug}:summary`, `person:{name}`, `daily:{date}`) that cannot be re-minted under a new id: a wrongly-flagged project summary would die forever while the distiller kept appending revisions that silently never shipped. New rule: **the winning revision is closed iff some close's `created_at` is strictly newer than that revision's `created_at`.** A newer revision (re-distill, `note edit`, or `revise_note`) re-opens a flagged *or* superseded note; a close minted at the same instant as its revision still closes it (the common supersession case); a close that predates the winning revision is void. This aligns the index with the note log's latest-wins philosophy (`latestRecordPerNoteId` already lets a newer revision revive a tombstone). Scope: supersession shares the mechanism, so revising a superseded note resurrects it alongside its replacement — deliberate, since only the human and the deterministic-ID distiller can revise (both deliberate acts). Recovery from a flag is therefore "a newer revision re-opens," not "supersede back." The `flag_note` tool description names the real path.


**Amendment since v4 (2026-08-04 — settle-eligibility session):**
- **§5 amended — settle-eligibility recorded as a structural invariant** (`docs/specs/structural-invariants.md` §4: *a session's delta is distillable only when settled*). It is the property that makes a global drain safe to fire from anywhere, at any time, and so the precondition under any automatic trigger. The trigger *mechanism* is not settled by this amendment and must not be cited as settled — it belongs to the story that lands it.
- **The settle window's default moved from 30 min to 24 h.** The 30 min figure was written while the window was still read as a quality threshold. It is not one — no value prevents a mid-arc split — so the value is deliberately not load-bearing and the asymmetry decides it: over-waiting costs almost nothing under this design, under-waiting distills live work. Old figure kept so the change is auditable.

**Amendment since v4 (2026-07-23 — packaging & guided-setup session):**
- **§14 amended — packaging graduated from deferred to active.** The Repo-bootstrap "decided when something forces the question (first external contributor, first publish)" trigger has fired. Breakdown/status live in a new GitHub epic (*Packaging & guided setup*); the durable rulings:
- **End-user distribution is a bundled single-executable, not `npm i -g`.** Reason: npm-global presupposes a Node install and inherits the native-ABI-vs-Node-version breakage class — `better-sqlite3` is ABI-keyed (`NODE_MODULE_VERSION`), so any Node major switch either hides the global install (nvm isolates globals per version) or hard-crashes it mid-session (shared/system Node) until a rebuild; `sqlite-vec` rides in as a platform-keyed loadable extension and is not the sensitive piece. A bundled runtime pins runtime+ABI together and is the only shape that delivers §1's "install once and forget" (the Ollama / SSH-agent / git-credential-manager precedents §1 invokes ship platform installers, not npm packages). **Supersedes the end-user reading of §14's "CLI via `package.json` `bin` → `dist/`"**: that is the dev/build shape; npm stays *build/dev tooling only*, never the user install path.
- **Single-executable feasibility is de-risked by a Node SEA PoC before any commitment.** The compiler (Node SEA vs `bun build --compile`) is a PoC *finding*, not a fixed choice — this concerns a build-time compiler only and does **not** reopen §14's Node-as-dev-runtime decision (Bun-as-runtime stays rejected). Open questions the PoC settles: ESM→CJS bundling (SEA takes a CJS entry; the code is ESM) and the native-artifact strategy (embed-as-asset-and-extract vs. ship sidecars) — the latter also decides the update-swap mechanism (single-file atomic replace vs. versioned-dir + symlink/junction flip).
- **Guided setup is a CLI wizard (`librarian init` / `librarian config`) — the universal engine across all surfaces** (Claude Code, OpenCode, standalone MCP; a slash command would serve only one). The menu is an *editor* over the canonical `~/.librarian/config.json` — file-over-app preserved (this is the answer to "a config menu betrays glass-box": the file stays canonical and hand-editable). **Two-layer config:** host-owned *wiring* (declarative plugin manifests / MCP registration) vs. librarian-owned *domain* config (the single file); Claude Code's `userConfig` is **rejected** as a domain-config home (per-host store, invisible to the other two surfaces, splits the canonical file). The curated menu covers provider / embedding / vault only and **excludes `scoring`** (expert tuning — no sensible-default picklist exists; it *is* the default).
- **All surfaces route through the single registered bin (thin plugins, not self-contained).** The Claude Code plugin declares its four hooks + the stdio MCP server in `.claude-plugin/plugin.json` (no user settings-editing); the per-agent hook-payload mapping moves behind a `librarian hook <agent>` subcommand so a thin plugin needs only the bin, not a bundled `dist/`. OpenCode plugin resolves the CLI via config `bin`. Rejected: self-contained plugins that each bundle `dist/` (three code copies, three path regimes — the opposite of one validated bin).
- **~~The OpenCode plugin ships as a dedicated GitHub-sourced package, registered in OpenCode's own config — not published to npm, not a copied/bundled local file (2026-07-27).~~ SUPERSEDED 2026-07-29 — see the bullet below.** Superseded because its premise ("the adapter is three source files plus a runtime dep") stopped being true once the mapping moved behind `librarian hook opencode`: what remains is one dependency-free file, and a file needs no package. The original text, kept whole so the reasoning is auditable: *OpenCode's two plugin-load mechanisms are (a) local files in `~/.config/opencode/plugins/` and (b) npm-style package specs in `opencode.json`, which OpenCode installs (via Bun) and caches. The local-file path is rejected: the adapter is three source files plus a runtime dep (`ulid`), and OpenCode loads flat plugin files individually (siblings each parsed as their own plugin, no dep resolution), so a real install would require *generating* a self-contained bundle and coupling Librarian's app build to that host-specific artifact — the "self-contained plugin bundles code" anti-pattern this section already rejects, in a new form. The package path is taken, sourced from Git rather than the npm registry: **npm-publish is refused** for the same reason §14 refuses `npm i -g` for the CLI (npm-registry publish is dev/distribution tooling this project doesn't need, and a published-package cadence is its own release surface). A **dedicated repo** (not a subdirectory of this one) is required because a git subdirectory package spec was **verified not to work** (Bun 1.3.14: `github:owner/repo#ref` installs a repo root, but every tested subdirectory form — `#ref:path`, `#path:…`, `#…&path=…` — is treated as part of the Git ref and 404s), and because sourcing the whole Librarian repo as one package would drag its entire dependency surface into every OpenCode session. The spec pins the *shape* (dedicated Git-sourced package, tag/commit-pinned, wizard edits `opencode.json`); the repo location and version-pinning cadence live with the work. The plugin stays thin — it still resolves the CLI via config `bin` = the installed binary, per the bullet above.*
- **The OpenCode plugin ships as one dependency-free file the wizard writes; its mapping lives behind `librarian hook opencode` (2026-07-29, supersedes the 2026-07-27 dedicated-repo ruling above).** The three host classes have different integration physics, and only one of them ever needed a package: Claude Code loads a *declarative* manifest (`.claude-plugin/plugin.json`, `command` hooks → spawn the bin — zero code shipped); Codex / Cursor / ChatGPT and every other MCP host takes a config entry → spawn `librarian mcp` (a bin, not a package — **nothing to build for them at all**); OpenCode alone loads **executable JS in-process**, so some code must land in the host. After the port that code is one file with **zero non-node imports** (`ulid` left with the mapper, exactly as it did for Claude Code) doing four things: hook subscriptions, spawn the bin with the raw native payload, cache the returned blocks, splice them. The wizard writes it to `~/.config/opencode/plugins/librarian.ts` and points config `bin` at the installed binary; the binary carries the file as a build asset (`sea-config.json`), so the installed path never reads a checkout. Why the plugin must ship *any* code: OpenCode has no `additionalContext` channel — recall is spliced in `experimental.chat.messages.transform`, mutating the outgoing message array inside OpenCode's own process, which a spawned command cannot do. **OpenCode's hook contract is "mutate `output` in place; return `void`"** (its own wording), and for the transform that means mutating the message *array object* — verified in 1.18.9, where the call site is `trigger('experimental.chat.messages.transform', {}, {messages: ze})` followed by `toModelMessagesEffect(ze, …)`: rebinding `output.messages` to a new array, or returning one, is silently dropped and the injection never reaches the model. Same for `experimental.session.compacting`, whose `context` arrives as an array to push onto. This is the load-bearing detail of the whole push path on this host and it is invisible to any test that asserts on a return value. **Why-nots, all rejected:** a *dedicated second repo* and a *repo inversion* (machinery around one file); a *CI-generated tag/branch of this repo whose root is a thin package* — its own case conceded there is "almost nothing left to version independently", which argues for not versioning it rather than for versioning it cheaply, and it adds a CI job, tag-namespace pollution, a plugin↔binary version skew that cannot exist today, and unverified Bun/OpenCode cache-invalidation semantics, while leaving the supported install exercised only in CI; *npm publish* (refused for the same reason §14 refuses `npm i -g`); a *generated self-contained bundle* (the anti-pattern this section already rejects). The transferable principle from Claude Code is **the wizard owns host wiring**, not "every host installs from a git ref" — writing `opencode.json` and writing a plugin file are the same act. Note this also settles the map/schema co-location argument the superseded ruling partly rested on: `tsc` compiles only `src/` (`rootDir=src`) and rewrites `.ts`→`.js`, so a `src/ → adapters/` *import* diverges between the dev build and the esbuild SEA bundle; both mappers therefore live in `src/hook/` and `adapters/<host>/` holds docs, fixtures, and (for OpenCode) the one plugin file. **Trigger that would make a package correct:** a *second* host that genuinely needs in-process code (a real Cursor/Zed/Aider plugin) — two call sites, then generalise. Not before.
- **Update / uninstall policy: guided and explicit, never silent.** Reason: librarian's code runs as hooks on every prompt and injects context invisibly — auto-changing that code under the user contradicts glass-box (§1). Passive "update available" check + explicit apply + post-swap `doctor` verify + rollback; auto-apply is host-owned and opt-in (e.g. Claude Code's own plugin update). Uninstall removes wiring and the bin but **preserves `~/.librarian` data by default** (the user's notes are theirs; removing the tool never removes the memory). **Mechanism, settled 2026-08-04** now that the SEA PoC landed single-file (#149) and the install layout is one flat `~/.librarian/bin/librarian` (#151): `librarian uninstall` removes exactly what an install wrote — that binary (plus any staged/rollback sibling a killed self-update stranded), the installer's marker-tagged `PATH` line from whichever profile got it, the one OpenCode plugin file, and the config keys that point at the bin (settings survive; `bin` is wiring, not preference). Host-owned wiring is **printed, never written**: the Claude Code plugin registry and hand-made `claude mcp add` entries live in the host's own config, which Librarian does not author (§14 amendment — the wizard owns *its* wiring), so guessing at another tool's state is refused in favor of naming the two commands. `--purge` deletes `~/.librarian` behind a confirmation; `--dry-run` reports without touching anything.
...
- **Secretlint-backed corpus (2026-08-03 → #178):** `redact()`/`redactOutput()` layer `@secretlint/core` + its recommended preset (28 rules — Anthropic, OpenAI, GCP, Slack, private keys, connection strings, and more) after the hand-rolled `SECRET_PATTERNS`, plus a pass that string-replaces this machine's own configured credential-named env values before either runs. The hand-rolled patterns are **kept, not replaced**: secretlint's `aws` rule only matches the secret-access-key shape and walks past a bare `AKIA…` key ID. The corpus is a **maintained dependency, not a hand-curated list** — keeping it current is `npm update`, not remembering to add a pattern.
...
- **A failing session never strands the backlog (2026-08-05).** A retryable per-session failure records its attempt on that session's cursor (offset unmoved) and the pass **continues**; the run throws one aggregate at the end, so the exit is still loud. Aborting at the first failure meant one malformed provider response held ~300 pending sessions hostage, and the sessions behind it in the walk are independent work — the cursor already carries the retry state that makes continuing safe. Durable-but-unpublished work is not held back either: the index pass and the vault export run before the aggregate throw.
- **Diagnosis is a file, not a logger (2026-08-05).** The failure record must carry what the failure cannot be reconstructed without: a provider failure names a bounded, redacted **excerpt of the offending response**, not just `SyntaxError: … position 637`, and it rides the error message into `failed_attempts.last_error` and the quarantine verdict — files that are always written. The only debug switch is `debug: true` in the config, and it adds only the **stack**, the one thing an error message structurally cannot carry. No log levels, no log file, no second output channel: a parallel logging surface would compete with the verdict/cursor logs for the truth. Config, not an env var — a switch nobody can find is a switch nobody uses.
- **Settle-eligibility: a session's delta is distillable only when settled (2026-08-04).** A delta is eligible only once its session has been quiet for `distill.settleMs` (default 24 h) or a terminal boundary marker says the arc is over; a held-back delta leaves the cursor untouched and emits a `deferred` diagnostic, never a `skipped` verdict. Full statement and enforcing mechanism: structural invariants §4. `settleMs: 0` is legal and turns the gate off — the operator keeps the last word over a rule that exists to protect them from a trigger they did not fire (file-over-app, §14). **What this decides is eligibility, not triggering** — `runDistill` sweeps every pending session, so *any* trigger would otherwise distill sessions that are still live. With eligibility carrying that weight, a drain is safe to fire from anywhere at any time, and firing during a live session is a cheap near-no-op for it while the settled sessions drain. The trigger mechanism is separate work and is not settled here.
  - **The settle window is a concurrency guard, not a quality knob.** No threshold prevents a mid-arc split — a real pause longer than it trips it, at any value — so its exact value is deliberately not load-bearing and the default is generous: over-waiting costs almost nothing here, under-waiting distills live work. Corollary: shortening the window is not a legitimate way to buy fewer splits, and the fragmentation question is measured before anything is absorbed.
  - **An OS scheduler would preserve the no-daemon ruling**, on the same precedent by which §15 delegates multi-machine sync to Syncthing/git/iCloud — "the sync layer is someone else's app; no daemon". It is someone else's scheduler, and each firing is a short-lived process that exits; the ruling refuses a *resident librarian process*, which that is not. A resident worker and a filesystem watcher stay rejected and keep their existing named trigger.
  - **Triggering, settled and shipped (2026-08-04 → #170): two triggers, no daemon.** A boundary marker (§10.1) spawns a **detached, unref'd `librarian drain`** from the hook shell *behind the bin*, so both adapters inherit one trigger instead of duplicating a spawn; `librarian install-schedule` writes the OS-native timer (launchd / systemd-user / cron) that covers what a hook cannot see — a hard-killed terminal, a crashed drain, a provider offline at session end, a machine asleep mid-run. Three properties of the spawn are load-bearing, each because the obvious alternative fails: **detached** (a hook that waited on a provider call would stall the host agent), **stdout-silent and unthrowing** (hook-safety, §14 — a spawn failure never propagates, and the scheduled net is its recovery path), and **debounced by a cache stamp** (a burst of boundaries would otherwise storm; the distiller lock is the backstop, but a lock collision still costs a spawned process). A *terminal* boundary bypasses the debounce: it is the firing that owes the session its note, and a commit seconds earlier would otherwise swallow it and defer the note to the timer. Overlap between the two triggers needs no coordination — a repeat drain over one backlog is a byte-identical no-op, so convergence is by construction rather than by scheduling. **What overlap did expose (2026-08-05):** the distiller lock covers the distill pass only, so the losing drain's *index* work was a second sqlite writer racing the holder — it now skips the distill and the index pass and exits 0, but still exports, because the holder may be a plain `librarian distill` that never exports and file writes race nothing.
...
[3] 12:07 bash: npm test → ✖ failing: 1 | stderr: Error: NODE_MODULE_VERSION 115 vs 127. …
[4] 12:09 bash: git commit -m "fix: expire check before redirect"  ← salient:vcs_commit
```

- **Tool outcome renders as a bounded excerpt (2026-07-31 → #179).** A non-zero `exit` is named on the line; a zero one is left implicit. A `command`/`vcs_*` event's captured `outcome` renders as at most `OUTCOME_EXCERPT_CHARS` of collapsed text **per stream**, stdout first and stderr labelled. Both streams render: measured over real transcripts a failing suite prints its failure to stdout (`2>&1 | tail`, `node --test`) while stderr carries harness noise, so preferring either one loses the lesson whenever it matters. Storage and prompt budget stay separate contracts — letting the budget dictate the schema is exactly what left failure unrecorded for 7266 events.
