...
- **Tool outcome renders as a bounded excerpt (2026-07-31 → #179).** A `command`/`vcs_*` event's captured `outcome` renders as at most `OUTCOME_EXCERPT_CHARS` of collapsed text **per stream**, stdout first and stderr labelled. Both streams render: measured over real transcripts a failing suite prints its failure to stdout (`2>&1 | tail`, `node --test`) while stderr carries harness noise, so preferring either one loses the lesson whenever it matters. Storage and prompt budget stay separate contracts — letting the budget dictate the schema is exactly what left failure unrecorded for 7266 events.
