...
**Amendment since v4 (2026-08-04 — automatic-drain session):**
- **§5 amended — distill triggering settled: a detached child on a real session boundary plus an OS-scheduled net, both gated by settle-eligibility.** §4 always assigned the collector "distill triggering (lazy, detached-child model)"; this names what pulls the trigger without reopening the no-daemon ruling. The eligibility half is a structural invariant (`docs/specs/structural-invariants.md` §4: *a session's delta is distillable only when settled*) — it is what makes a global drain safe to fire from anywhere.
...
- **Distill triggering: boundary child + OS-scheduled net, gated by settle (2026-08-04).** Two triggers, both short-lived processes that exit: a **detached, unref'd `librarian drain`** spawned when an adapter sees a real session boundary, and an **OS-native timer** (launchd / systemd-user / cron) written by an explicit `librarian install-schedule` for everything the hook cannot cover — hard-killed terminal, crashed drain, provider offline at session end, machine asleep. **An OS scheduler preserves the no-daemon ruling**, on the same precedent by which §15 delegates multi-machine sync to Syncthing/git/iCloud ("the sync layer is someone else's app; no daemon"): it is someone else's scheduler, and each firing is a process that ends. A resident worker and a filesystem watcher stay rejected and keep their existing named trigger. Wiring is guided and explicit, never silent (§14 packaging).
  - **What makes a liberal trigger safe is eligibility, not trigger discipline.** `runDistill` sweeps every pending session, so any trigger whatsoever would otherwise distill sessions that are still live. The settle invariant (structural invariants §4) carries that weight: a delta is distillable only once its session has been quiet for `distill.settleMs` (default 24 h) or a terminal boundary marker says the arc is over; a held-back delta leaves the cursor untouched and emits a `deferred` diagnostic, never a `skipped` verdict. Firing mid-session is therefore a cheap near-no-op for the live session while opportunistically draining the settled ones — which is the property that lets the trigger be liberal instead of clever.
  - **The settle window is a concurrency guard, not a quality knob.** No threshold prevents a mid-arc split (a real pause longer than it always trips it, at any value), so its exact value is deliberately not load-bearing and the default is generous: over-waiting costs almost nothing here, under-waiting distills live work. Consequently the trigger design is not permitted to buy fewer splits by shortening it; the fragmentation question is measured before anything is absorbed.
