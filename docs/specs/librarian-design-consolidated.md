...
- **Issue shape for parallel agents (2026-08-01 — tightens the bullet above, does not replace it).** The issue body is a *bounded execution contract*, not the reasoning: fixed field order (**Outcome · Why · Touchpoints · Constraints · Success signals · Context**), ≤400 tokens for a task and ≤700 for a story. `AGENTS.md` → "Issue shape" is the **sole** normative statement of that contract, split from a short "Tracker bindings" section holding the GitHub mechanisms (label, sub-issues, relationship field, Project v2, the templates) so the procedure ports to another tracker by rewriting five lines. `.github/ISSUE_TEMPLATE/{task,story}.yml` is prefill only and deliberately states no rules: a template that restates the contract is a second copy nothing reconciles, and it constrains nothing created through the API, which is the dominant authoring path. Two things this settles that the original bullet left implicit. **Touchpoints are mandatory** — a declared file set is both the shortcut past the agent's opening repo-wide search and the disjointness key deciding which sibling tasks may run concurrently, which makes parallelism a property of the issue graph rather than of whoever dispatches it; overlap is a `blocked by` edge. And **durable reasoning leaves the body** — evidence tables, design forks and superseded approaches move to `docs/specs/` or `docs/research/` and are linked. That is this project's own routing test applied to its issues instead of merely described by them, and the depth is not deleted but re-layered: it now outlives the closed issue, where before it died with it. The forcing argument is that an issue body is replayed as prompt prefix on every turn, so its cost is not paid once — and length degrades instruction adherence before it degrades anything else, which makes an over-long issue a *silent requirement loss*, not merely an expensive one. Rejected: a template without the `AGENTS.md` contract (forms do not constrain API-created issues, the dominant authoring path here); one template covering epics too (an epic has no touchpoints or success signals of its own, so the fields become filler the agent must read past); `<details>` blocks to hide depth (zero saving for an agent — it hides the problem from the human reviewer instead); hoisting shared context up into the parent story (breaks the property that a task is executable without reading its parent, which is exactly what makes parallel dispatch possible); prescriptive step lists as a safety measure (over-specification destabilizes agents — steps get either ignored or followed literally). CI validation of the required fields is deferred behind a named trigger: the first issue that reaches an agent with an empty Touchpoints. Evidence and the reasoning behind the budget: `docs/research/agentic-issue-template.md`.
