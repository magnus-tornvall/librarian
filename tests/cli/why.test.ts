...
test('why reads real pull and push traces, supports JSON, and handles missing traces', () => {
  ...
  const inject = runCli(['inject', '--project', 'alpha', '--data-dir', t.dataDir, '--diagnostics-dir', t.diagnosticsDir], t.indexDir, 'narwhal');
...
test('why-not explains floor, scope, and BM25 misses without writing diagnostics', () => {
  ...
  const floor = runCli(['why-not', 'commonfloor', 'fact:why-0', '--project', 'alpha', '--data-dir', t.dataDir], t.indexDir);
  ...
  const scope = runCli(['why-not', 'narwhal', 'fact:why-scope', '--project', 'alpha', '--data-dir', t.dataDir], t.indexDir);
  assert.equal(scope.status, 0, `why-not scope should exit 0; stderr: ${scope.stderr}`);
  assert.match(scope.stdout, /Gate: scope_mismatch/);

  const miss = runCli(['why-not', 'narwhal', 'fact:why-miss', '--project', 'alpha', '--data-dir', t.dataDir], t.indexDir);
...
test('why-not reports the undamped score for a year-old decision', () => {
  ...
  const result = runCli(['why-not', 'permanent credential', 'decision:old-permanent', '--project', 'alpha', '--data-dir', t.dataDir], t.indexDir);
...
test('expired episodes remain in the note log while recall, why-not, and trace replay name ttl_expired', () => {
  ...
  const recall = runCli(['recall', 'walrus ttl', '--project', 'alpha', '--data-dir', t.dataDir, '--diagnostics-dir', t.diagnosticsDir], t.indexDir);
  assert.equal(recall.status, 0, `recall should exit 0; stderr: ${recall.stderr}`);
  ...
  const whyNot = runCli(['why-not', 'walrus ttl', 'episode:expired-walrus', '--project', 'alpha', '--data-dir', t.dataDir], t.indexDir);
...
test('why-not budget gate matches the pull-path limit of 10, not the scoring cap of 5', () => {
  ...
  const recall = runCli(['recall', 'narwhal', '--project', 'alpha', '--limit', '10', '--json', '--data-dir', t.dataDir, '--diagnostics-dir', t.diagnosticsDir], t.indexDir);
  assert.equal(recall.status, 0, `recall should exit 0; stderr: ${recall.stderr}`);
  ...
  for (const noteId of shipped) {
    const result = runCli(['why-not', 'narwhal', noteId, '--project', 'alpha', '--data-dir', t.dataDir], t.indexDir);
    assert.equal(result.status, 0, `why-not should exit 0; stderr: ${result.stderr}`);
