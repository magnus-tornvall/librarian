...
| `PostToolUse` `Bash`                 | `ToolEvent`     | bash / command, `command` populated from `tool_input.command`; `git commit` / `git push` detection sharpens the category to `vcs_commit` / `vcs_push`. `outcome` (`stdout`/`stderr`/`interrupted`) is lifted from `tool_response`, empty streams elided; a non-empty `stderr` or `interrupted` sets `hints.possibly_salient` (`reason: command_failed`), which outranks the file_write/vcs_commit hints. |
